...
export default function Keywords(){
 ...
 async function researchSerp(k:string){setLoading(k);setError("");setResearch(null);try{const r=await fetch("/api/research",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:k})});const text=await r.text();let j:any;try{j=JSON.parse(text)}catch{throw new Error(text.slice(0,300)||`Falha HTTP ${r.status}`)}if(!r.ok)throw new Error(j.error||`Falha HTTP ${r.status}`);setResearch(j.research)}catch(e){setError(e instanceof Error?e.message:"Erro na pesquisa.")}finally{setLoading("")}}
